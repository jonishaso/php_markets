...
class mdl_infoClass extends mdl_base
{
	...
	public function getChild2 ($class_id, $level = 20)
	{
		...
		$re = $this->db->select('id, ordinal, name, nameEn, url, defaultDisplayMode, content', $this->tableName, $where, "ordinal asc");
		while ($ro = $this->db->fetch_array($re))
		{
			$childCount	= $this->getChildCount($ro['id']);
			$classArray[$ro['id']] = array(
	...
	{
		return $this->db->selectOne('id,name', $this->tableName, array('id'=>$id));
	}

	public function getAboutusContentById ($id)
	{
		return $this->db->selectOne('content', $this->tableName, array('id'=>$id));
	}
