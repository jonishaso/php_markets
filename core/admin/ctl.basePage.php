...
class adminPage extends corecms
{
	...
	protected function getAllPermissionClass ($ctl, $id, $action = 'index/index')
	{
		...
		}else if(empty(session('admin_flag'))){
			$infoclass		= $mdl_infoClass->getChild($mdl_infoClass->getByAlias($ctl)['id']);
		}else{
			$infoclass		= $mdl_infoClass->getChild($mdl_infoClass->getByAlias($ctl)['id']);
		}
		$array			= array();
