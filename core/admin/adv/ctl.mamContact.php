...
/*
 @ctl_name = mamContact管理@
*/

class ctl_mamContact extends adminPage
{

	public function index_action () #act_name = mamContact列表#
	{
		date_default_timezone_set('Australia/Sydney');
		$bllLink	= $this->loadModel('mamContact');
		$where		= "";
		...
		if ( trim( get2( 'cmd' ) ) == 'export' ) {
		    ...
		    $excel->getActiveSheet()->setCellValue('F1', "Area");
		    $excel->getActiveSheet()->setCellValue('G1', "Message");
		    $excel->getActiveSheet()->setCellValue('H1', "Message Time");
	        	...
	        	$num=$k+2;
	             $excel->setActiveSheetIndex(0)
	                         ...
	                         //Excel的第A列，uid是你查出数组的键值，下面以此类推
	                          ...
	                          ->setCellValue('E'.$num, $v['country'])
	                          ->setCellValue('F'.$num, $v['area'])
	                          ->setCellValue('G'.$num, $v['message'])
	                          ->setCellValue('H'.$num, date('Y-m-d H:i:s', $v['createtime']));
	        }
	...
	public function edit_action () #act_name = mamContact回复#
	{
		$id			= (int)get2('id');
		$bllLink	= $this->loadModel('mamContact');
		$data		= $bllLink->get($id);
	...
	public function delete_action () #act_name = 删除mamContact#
	{
		if (is_post())
		{
			$ids = post('ids');
