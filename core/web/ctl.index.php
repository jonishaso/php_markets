...
class ctl_index extends cmsPage
{
	...
	function index_action() {
		...
		if($this->getLangStr() == 'en'){
			$logo = $infopic->getList(3);
			$this->setData($logo,'logo');
			$this->setData($infopic->getList(5),'banners');
			$this->setData($infopic->getList(6),'logoredirect');
		}else{
			$logo = $infopic->getList(108);
			$this->setData($logo,'logo');
			$this->setData($infopic->getList(109),'banners');
			$this->setData($infopic->getList(110),'logoredirect');
		}
		...
		fputs($ptr,'W');
		fputs($ptr,"WAPQUOTES-EURUSD,GBPUSD,USDCHF,AUDUSD,USDJPY,XAUUSD,XAGUSD,XAGUSD\n");
		$buf ="";
		$symbol = array();
		$symbols = array();
		while(!feof($ptr))
		...
		// $this->setData($this->site['description'], 'pageDescription');
		// $this->setData( 'default', 'menu' );
		// $this->setData($_GET['person'],'person');
		$this->display('default');
	}
